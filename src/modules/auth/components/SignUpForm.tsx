...
export function SignUpForm({ intent }: SignUpFormProps) {
  ...
  return (
    <>
      <h2 className="text-2xl font-semibold tracking-tight text-foreground text-center">Create an account</h2>
      <p className={`mt-2 text-sm font-light text-center ${textMuted}`}>Start your journey with Milly.</p>

      <form onSubmit={handleSubmit} className="mt-8 flex flex-col gap-4">
        <div className="grid grid-cols-2 gap-3">
          <div className="flex flex-col gap-1.5">
            <label htmlFor="firstName" className={`text-xs font-medium ${textMuted}`}>
              First Name
            </label>
            <input
              id="firstName"
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="lastName" className={`text-xs font-medium ${textMuted}`}>
              Last Name
            </label>
            <input
              id="lastName"
      ...
      <p className={`mt-6 text-center text-sm ${textMuted}`}>
        ...
        <Link href={authLink("/login", intent)} className="font-medium text-foreground hover:underline">
          Sign In
        </Link>
      </p>
    </>
